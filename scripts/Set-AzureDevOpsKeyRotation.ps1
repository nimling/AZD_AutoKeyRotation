...
if ($serviceConnection.data.creationMode -eq "Automatic") {
    Write-Host "Service connection creation mode is: Automatic"
    $serviceConnection.data.PSObject.Properties.Remove("spnObjectId")
    $serviceConnection.data.PSObject.Properties.Remove("appObjectId")
    $serviceConnection.data.creationMode = "Manual"
}    
else {
    Write-Host "Service connection creation mode is: Manual"   
}
$serviceConnection.authorization.parameters.servicePrincipalKey = $newPassword.secretText
$serviceConnection.isReady = $false
$params = @{
    "Method"  = "Put"
    "Uri"     = "$($baseUri)/_apis/serviceendpoint/endpoints/$($serviceConnection.id)?api-version=6.1-preview.4"
    "Headers" = $headerDevOps
    "Body"    = $serviceConnection | ConvertTo-Json -Compress -Depth 99
}
$serviceConnection = Invoke-WebRequest @params 
...
$passwordsToRemove = $application.passwordCredentials | Where-Object -FilterScript { $_.keyId -ne $newPassword.keyId -and $_.customKeyIdentifier -eq "Auto" } | Sort-Object -Property startDateTime | Select-Object -Skip 1
